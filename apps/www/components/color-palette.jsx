...
export function ColorPalette(_a) {
    var colorPalette = _a.colorPalette;
    return (<div id={colorPalette.name} className="rounded-lg shadow-sm ring-1 ring-border">
      ...
      <div className="flex flex-col gap-1 p-2 sm:flex-row sm:gap-2">
        {colorPalette.colors.map(function (color) { return (<Color key={color.hex} color={color}/>); })}
      </div>
    </div>);
}
