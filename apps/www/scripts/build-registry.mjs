...
async function syncStyles() {
    ...
    for (const item of registry.items) {
        ...
        const resolveFiles = (_a = item.files) === null || _a === void 0 ? void 0 : _a.map((file) => `registry/${sourceStyle}/${typeof file === "string" ? file : file.path}`);
        if (!resolveFiles) {
            continue;
        }
...
`;
    for (const style of styles) {
        ...
        // Build style index.
        for (const item of registry.items) {
            const resolveFiles = (_a = item.files) === null || _a === void 0 ? void 0 : _a.map((file) => `registry/${style.name}/${typeof file === "string" ? file : file.path}`);
            if (!resolveFiles) {
                continue;
            }
            // Validate categories.
            ...
            if (item.type === "registry:block") {
                ...
                if (item.files) {
                    const files = item.files.map((file) => typeof file === "string"
    ...
    "${item.name}": {
      ...
      files: [${(_c = item.files) === null || _c === void 0 ? void 0 : _c.map((file) => {
                var _a;
        ...
        path: "${filePath}",
        type: "${file.type}",
        target: "${(_a = file.target) !== null && _a !== void 0 ? _a : ""}"
      }`;
            })}],
...
`;
    ...
    const items = registry.items
        ...
        .map((item) => {
        var _a;
        return Object.assign(Object.assign({}, item), { files: (_a = item.files) === null || _a === void 0 ? void 0 : _a.map((_file) => {
                const file = typeof _file === "string"
                    ? {
                        path: _file,
                        type: item.type,
                    }
...
// ----------------------------------------------------------------------------
async function buildStyles(registry) {
    for (const style of styles) {
        ...
        for (const item of registry.items) {
            ...
            let files;
            if (item.files) {
                files = await Promise.all(item.files.map(async (_file) => {
                    var _a, _b, _c;
                    const file = typeof _file === "string"
                        ? {
                            path: _file,
                    ...
                    (_a = sourceFile.getVariableDeclaration("iframeHeight")) === null || _a === void 0 ? void 0 : _a.remove();
                    (_b = sourceFile.getVariableDeclaration("containerClassName")) === null || _b === void 0 ? void 0 : _b.remove();
                    (_c = sourceFile.getVariableDeclaration("description")) === null || _c === void 0 ? void 0 : _c.remove();
            ...
            const payload = registryItemSchema.safeParse(Object.assign(Object.assign({ $schema: "https://ui.shadcn.com/schema/registry-item.json", author: "shadcn (https://ui.shadcn.com)" }, item), { files }));
            if (payload.success) {
                ...
                styleRegistryItems.push(Object.assign(Object.assign({}, item), { files: files === null || files === void 0 ? void 0 : files.map((file) => {
                        if (!file)
                            return null;
                        // Exclude content from registry.json
                        const { content } = file, fileWithoutContent = __rest(file, ["content"]);
                        return Object.assign(Object.assign({}, fileWithoutContent), { path: `registry/${style.name}/${file.path}` });
                    }).filter(Boolean) }));
            }
        }
        ...
        if (validatedRegistry.success) {
            ...
            await fs.writeFile(path.join(targetPath, "registry.json"), styleRegistryJson, "utf8");
        }
        else {
            console.error(`Failed to validate registry for style ${style.name}:`, validatedRegistry.error);
        }
    }
    // ----------------------------------------------------------------------------
...
async function buildThemes() {
    ...
    for (const [color, value] of Object.entries(colors)) {
        ...
        if (Array.isArray(value)) {
            colorsData[color] = value.map((item) => (Object.assign(Object.assign({}, item), { rgbChannel: item.rgb.replace(/^rgb\((\d+),(\d+),(\d+)\)$/, "$1 $2 $3"), hslChannel: item.hsl.replace(/^hsl\(([\d.]+),([\d.]+%),([\d.]+%)\)$/, "$1 $2 $3") })));
            continue;
        }
        if (typeof value === "object") {
            colorsData[color] = Object.assign(Object.assign({}, value), { rgbChannel: value.rgb.replace(/^rgb\((\d+),(\d+),(\d+)\)$/, "$1 $2 $3"), hslChannel: value.hsl.replace(/^hsl\(([\d.]+),([\d.]+%),([\d.]+%)\)$/, "$1 $2 $3") });
            continue;
        }
    }
  ...
  * {
  ...
  body {
    ...
    for (const baseColor of ["slate", "gray", "zinc", "neutral", "stone"]) {
        ...
        // Add v4 css vars.
        base["cssVarsV4"] = baseColorsV4[baseColor];
        // Build css vars.
